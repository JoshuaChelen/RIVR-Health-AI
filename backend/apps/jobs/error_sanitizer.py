...
def sanitize_log_message(message, max_length: int = 500) -> str:
    """Return ``str(message)`` with credentials AND PHI redacted, capped at max_length.

    Use this for AiJobEvent.message/data and Document.processing_error — anywhere
    exception text is persisted to the database or returned in API responses.
    """
    text = message if isinstance(message, str) else str(message)
